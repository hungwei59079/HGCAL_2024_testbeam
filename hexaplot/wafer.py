...
def fill_wafer_hist(ch_values, moduletype="ML_L"):
    ...
    for key in file.GetListOfKeys():
        ...
        if idx < len(ch_values):
            ...
            raise ValueError(
                f"Length of values {len(ch_values)} does not accomodate for #obj={iobj} eRx={eRx} idx={idx}"
            )
        iobj += 1
...
def draw_and_save(hist, title, **kwargs):
    minimum = kwargs.get("minimum", 0)
    maximum = kwargs.get("maximum", 5)
    PaintFormat = kwargs.get("PaintFormat", ".2f")
    output_dir = kwargs.get("output_dir", ".")

    if maximum < minimum:
        raise ValueError(
            f"Invalid range: maximum ({maximum}) is smaller than minimum ({minimum})"
        )
